...
def color_arcs(C, predictions):
    ...
    for i, conf in enumerate(predictions):
        ...
        arc = C.create_arc(coord, start=i * UI_RESOLUTION - 5, extent=UI_RESOLUTION, fill=fill, outline=outline, width=2)
        arcs.append(arc)
...
def create_labels():
    ...
    az_conf_label, az_conf_val, azimuth_label, azimuth_val = create_doa_labels(
        'Azimuth 1', value_font_size, x, x_shift, y_azimuth, y_shift)

    y_elevation = 250
    el_conf_label, el_conf_val, elevation_label, elevation_val = create_doa_labels(
        'Azimuth 2', value_font_size, x, x_shift, y_elevation, y_shift)
